...
def get_three_node_graphlet_dist_adj_list_v2(G: nx.MultiDiGraph, G_prime: nx.Graph):
    ...
    node_list = sorted(G.nodes(), key=lambda node: G.degree(node))
    ...
    #     print(f"{node} {G.degree(node)} {node_index[node]}")
    ...
    for i in node_list:
        ...
        for j, k in get_combinations(sorted_neighbors):
            ...
            if hash_val not in three_node_graphlet_dict:
                three_node_graphlet_dict[hash_val] = 0
                graphlet_mapper[hash_val] = sorted_tuples
                hashed = True
            three_node_graphlet_dict[hash_val] += 1

            # print(completed_i)
        # print(f"{i} : {local_graphlet_dict}")
        # if i not in completed_i:
        #     completed_i.add(i)
        print(f"{i}", end="\r")
...
def main(ppi_path, reg_path, node_limit, edge_limit):
    ...
    for graphlet_hash, count in graphlet_dict.items():
        ...
        graphlet_count+=count
    print(f"unique graphlets found : {len(graphlet_dict)}")
    print(f"total graphlets found : {graphlet_count}")
